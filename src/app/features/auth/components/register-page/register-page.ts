  ...
  private register() {
    ...
    this.subscription = this.authService.register({
      ...
      next: () => {
        ...
        this.displayAlert('تم إنشاء الحساب بنجاح! جاري تحويلك لتسجيل الدخول', 'success');
      ...
      error: (err) => {
        this.isLoading = false;
        // عرض رسالة الخطأ من الخادم إن وجدت، وإلا رسالة عامة
        const message = err.error?.message || 'حدث خطأ أثناء إنشاء الحساب';
